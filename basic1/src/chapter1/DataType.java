...
public class DataType {
	...
	public static void main(String[] args) {
		...
		// 1. 문자열 결합
		// 2. + 혹은 concat(문자열)으로 두 문자열을 연결할 수 있음
		System.out.println(statement + "@@@");
		System.out.println(statement .concat("!!!"));
		
		// 2. 문자열 비교
		// equals(문자열): 두 문자열이 동등한지 비교
		// compareTo (문자열): 두 문자열을 사전순으로 비교 (대쇼문자 구분)
		// compareTo(문자열): 두 문자열을 사전으로 비교(대소문
		// compareToIgnoreCase (뮨저욜): 두 문자열을 사전순으로 비교 (대소문자 구분X)
		System.out.println(statement.equals(" My name is loki "));
		System.out.println(statement .compareTo("Zy name is loky "));
		...
		// lastIndesOf(문자열): 찾고자하는 문자열의 마지막 위치의 인덱스 번호를 반환
